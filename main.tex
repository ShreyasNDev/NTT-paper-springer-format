...
\titlerunning{Reducing Transform Count in NTT Product Chains}
\author{Shreyas Nathani \and Dr T. Appala Naidu}
\authorrunning{Nathani and Naidu}
...
The Number Theoretic Transform (NTT)~\cite{pollard1971fft} provides efficient algorithms for polynomial multiplication over finite fields and rings, reducing the cost from $O(n^2)$ schoolbook multiplication to $O(n \log n)$. It is a core operation in lattice-based cryptography, where schemes such as Kyber~\cite{bos2018kyber} and Dilithium~\cite{ducas2018dilithium} rely on polynomial multiplication in quotient rings of the form $\mathbb{Z}_q[x]/(x^n + 1)$ for key generation, encryption, and signature verification.
...
Available at \url{https://eips.ethereum.org/EIPS/eip-4844}
