...
\usepackage{xcolor}
\usepackage{url}
\title{Reducing Transform Count in NTT-Based Product Chains}
\author{Shreyas Nathani \and Dr T. Appala Naidu}
\institute{University of Hyderabad, Hyderabad, India}
...
\begin{abstract}
We describe an optimization for computing the product of k polynomials in a fixed quotient ring $Z_{q}[x]/(x^n+1)$ where \textbf{$q$} is NTT-friendly. Standard approaches require $3(k-1)$ Number Theoretic Transforms, but by performing all intermediate multiplications as Hadamard products in the spectral domain and deferring the inverse transform, we reduce this to $k+1$ transforms. We prove the correctness of this method via a generalized convolution theorem, analyze complexity, and present a CRT-based extension that applies the optimization to general polynomial multiplication where intermediate products grow in degree.
\keywords{Number Theoretic Transform \and Polynomial Multiplication \and Lattice-Based Cryptography \and Chinese Remainder Theorem \and Spectral Domain}
\end{abstract}


\section{Introduction}

The Number Theoretic Transform (NTT)[5] provides efficient algorithms for polynomial multiplication over finite fields and rings, reducing the cost from $O(n^2)$ schoolbook multiplication to $O(n \log n)$. It is a core operation in lattice-based cryptography, where schemes such as Kyber[1] and Dilithium[2] rely on polynomial multiplication in quotient rings of the form $\mathbb{Z}_q[x]/(x^n + 1)$ for key generation, encryption, and signature verification.

A single polynomial multiplication via the NTT requires two forward transforms, one pointwise (Hadamard) product, and one inverse transform. When a chain of $k$ polynomials must be multiplied, the standard approach applies this three-transform pattern to each of the $k - 1$ individual multiplications, resulting in $3(k-1)$ transforms in total.

We observe that this standard approach is wasteful when all polynomials reside in the same fixed-size quotient ring, as is the case in the cryptographic settings described above. Each intermediate result is transformed back to the coefficient domain via an inverse NTT, only to be immediately transformed forward again for the next multiplication. These intermediate inverse-forward pairs are redundant: since the Hadamard product of two spectral vectors is itself a valid spectral vector in the same ring, all intermediate multiplications can be performed entirely in the NTT domain, and the inverse transform can be deferred to the very end of the computation. 

The closest prior work is Saldamlı and Koç's Spectral Modular Exponentiation[4], which performs modular exponentiation entirely in the spectral domain. They note the same elimination of redundant transforms in a multiplicative chain. However, their method targets arbitrary RSA moduli, requiring Spectral Modular Reduction machinery to handle intermediate coefficient growth. Our setting is fundamentally simpler: over NTT-friendly rings, no intermediate reduction is needed, and the cancellation follows directly from the invertibility of the NTT.

We state the generalized convolution theorem for $k$-way products, analyze the precise transform savings, identify the exact conditions under which the optimization applies (and does not), and provide experimental validation.

We emphasize that this optimization applies specifically to rings where the NTT size is fixed across all multiplications. That is, where no intermediate degree growth occurs. In contrast, for general integer or polynomial multiplication where intermediate products grow in degree, the delayed inverse provides no benefit. For this, we use CRT reconstruction to apply our method, and thus achieve efficiency across multiplication without modulus. 

\paragraph{Organization.} Section 2 reviews the Number Theoretic Transform, the convolution theorem, and the Chinese Remainder Theorem. Section 3 presents the standard pairwise and balanced product tree algorithms for chain multiplication. Section 4 presents the Delayed Inverse Algorithm, the Generalized Convolution Theorem, the Hybrid Delayed Inverse Product Tree, and the General Delayed Inverse Algorithm. Section 5 provides experimental evaluation. Section 6 discusses an application to batch KZG opening. Section 7 concludes.

\section{Background}
\subsection{Number Theoretic Transforms and CRTs}
\begin{definition}

    The Number Theoretic Transform[5] is a variant of the discrete Fourier transform, where the root of unity is not complex but instead an integer over a modular ring. It can be seen as an exact discrete Fourier transform due to not requiring floating-point precision. Consider a polynomial with coefficients over the ring $\mathbb{Z} / q\mathbb{Z}$. We express this polynomial $a_0+a_1x+a_2x^{2}... +a_{n-1}x^{n-1}$ in the vector form.
$$A=\begin{pmatrix}
a_0 \\
a_1 \\
\vdots \\
...
\begin{definition}
    We say that a certain choice of modulus is NTT-possible if some nth primitive root of unity exists over the modulus choice. If

    $$\omega^n\equiv1 \pmod{q}$$
    $$\omega^k\not\equiv 1, k<n \pmod{q}$$

For this to be possible the modulus must be such that, if the modulus is decomposed to its prime factors
$$q=p_0^{m_0}\cdotp p_1^{m_1}\cdotp p_2^{m_2}...\cdotp p_{k}^{m_{k}} $$

then, for the nth root of unity to exist, n must divide $GCD(p_{0}-1,p_1-1,...p_k-1)$
\end{definition}
...
Defining the NTT and NTT-friendly moduli as such allow us to create simplifications akin to the Cooley-Tukey FFT [6].
...
where the INTT is the Inverse Number Theoretic Transform[7], given by
$$a_i=\frac{1}{n}\,\psi^{-i}\sum_{j=0}^{n-1}\hat{a}_{j}\,\omega^{-ij}$$
\end{definition}

This is the main idea \textbf{behind} Fourier transform based multiplication. As a convolution of vectors is identical to polynomial multiplication, performing Hadamard products over an NTT domain and then performing an Inverse NTT results in polynomial multiplication.
...
\begin{definition}
    ...
    The Chinese Remainder Theorem states that $A(x)$ has a unique solution $\pmod{Q}$, $Q = q_0 \cdotp q_1 \cdotp q_2 \dots q_{c-1}$. This is a consequence of the general Chinese Remainder Theorem~[8]. Define
    $$Q_i = \frac{Q}{q_i}$$
    $$N_i \equiv Q_i^{-1} \pmod{q_i}$$
    Then the unique solution $\pmod{Q}$ is given by
    $$\sum_{i=0}^{c-1} A_i(x)\, Q_i N_i \pmod{Q}$$
\end{definition}
\section{\textbf{Standard Chain Multiplication Algorithms}}
...
\subsection{\textbf{Balanced Product Tree}}
...
We recognize that this process can be repeated for all vectors. Thus, the convolution of all vectors can be found simply by \textbf{the following} generalized convolution theorem\textbf{:}
...
\begin{proof}
For any $a, b \in \mathbb{Z}[x]/(x^n+1)$, the $j$-th coefficient of $a \cdot b$ is a signed sum of at most $n$ terms of the form $a_i\,b_{(j-i)\bmod n}$, so
$$\|a \cdot b\|_\infty \leq n\,\|a\|_\infty\,\|b\|_\infty.$$
Applying this bound $k-1$ times to $p = a_0 \cdot a_1 \cdots a_{k-1}$, with each $\|a_i\|_\infty \leq B$, gives $\|p\|_\infty \leq n^{k-1}B^k$.
\end{proof}
...
All experiments were conducted on a Intel(R) Core(TM) Ultra 7 155H processor with 16 GB of RAM, running Microsoft Windows 11 Home 10.0.26200. The algorithms were implemented in Python 3.13, using only standard libraries and NumPy for array operations. No external NTT libraries were used; all four algorithms share the same iterative Cooley-Tukey NTT and Gentleman-Sande INTT implementations.
...
Correctness was verified on every trial by asserting that all four algorithms---Pairwise, Balanced Product Tree, Delayed Inverse, and Hybrid Delayed Inverse Product Tree---produce identical coefficient vectors. No mismatches were observed across all experiments.
...
The Hybrid Delayed Inverse Product Tree performs comparably to the sequential Delayed Inverse in single-threaded execution, as expected. The two algorithms perform the same number of transforms and Hadamard products; the Hybrid variant offers no serial advantage, but would exhibit a shorter critical path under parallel execution ($\log_2 k + 2$ versus $k + 1$).
...
\subsubsection*{Required CRT Moduli}

Figure~\ref{fig:exp2_c} shows the minimum number of CRT moduli $c$ required for each chain length. As $k$ increases, the bound $n^{k-1} B^k$ grows exponentially, requiring progressively more moduli to cover it. Each new modulus also multiplies the total transform count (both algorithms scale with $c$), so the CRT modulus count is the primary driver of the absolute running time for large $k$.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{experiment2_c.png}
    \caption{Minimum number of CRT moduli $c$ required to represent the product exactly, as a function of chain length $k$ ($n = 256$, $B = 2^{16}$).}
    \label{fig:exp2_c}
\end{figure}
...
\input{experiment2_table.tex}


\section{Application: Batch KZG Opening}

KZG polynomial commitments~\cite{kate2010kzg} commit to a polynomial $f(x)$ as a single elliptic-curve point $[f(\tau)]_1 = f(\tau) \cdot G_1$, where $\tau$ is a secret evaluation point from a one-time trusted setup. A single opening proof at point $z$ demonstrates $f(z) = y$ by exhibiting the quotient $q(x) = (f(x)-y)/(x-z)$ as a second curve point $\pi = [q(\tau)]_1$, verified by one pairing equation.

\subsection*{Batch Opening and the Zero Polynomial}

In deployed ZK-rollup systems and Ethereum's data availability layer (EIP-4844~\cite{eip4844}), it is common to open $t$ polynomials $f_1,\ldots,f_t$ at $t$ distinct arbitrary evaluation points $z_1,\ldots,z_t$ in a single aggregated proof. The standard batch-opening protocol~\cite{kate2010kzg} requires computing the \emph{zero polynomial}
$$Z(x) = \prod_{i=1}^{t}(x - z_i),$$
a degree-$t$ polynomial whose roots are exactly the evaluation points. $Z$ is used to construct the batched quotient: since $(x-z_i)$ divides $Z(x)$ for each $i$, all $t$ openings reduce to two curve points regardless of $t$.

Because the $z_i$ are arbitrary field elements chosen by the verifier (not elements of a fixed multiplicative subgroup), no closed form for $Z$ exists; it must be computed as an explicit product chain of $t$ degree-$1$ factors. In EIP-4844, $t$ reaches $4096$ per sampled blob; in the planned full Danksharding extension, $t$ rises to $8192$.

\subsection*{Fixed-Size NTT Condition}

The computation is performed over the scalar field $\mathbb{F}_p$ of BLS12-381, which satisfies $2^{32} \mid p - 1$ and is therefore NTT-friendly for transform sizes up to $2^{32}$. Choose any $n$ with $n > t$ and $n \mid p-1$ (e.g., the smallest power of two exceeding $t$). Zero-pad each factor $(x - z_i)$ to an $n$-coefficient vector. Because the final product $Z$ has degree $t < n$, no cyclic wraparound occurs under a size-$n$ convolution, and every multiplication produces the exact non-cyclic result. Since $n$ is fixed for the entire computation, the fixed-ring condition of the Delayed Inverse Algorithm is satisfied.

\subsection*{Transform Count and Speedup}

Under a balanced product tree (the standard approach), computing $Z(x)$ requires $3(t-1)$ size-$n$ NTTs. The Delayed Inverse Algorithm reduces this to $t+1$ transforms: $t$ forward NTTs (one per factor) and a single inverse NTT at the end. The speedup ratio is $3(t-1)/(t+1)$, approaching $3\times$ asymptotically. At the EIP-4844 parameters ($t = 4096$):
$$\frac{3(t-1)}{t+1} = \frac{3\times4095}{4097} \approx 2.9985\times.$$
At $t = 8192$ (full Danksharding): $\approx 2.9990\times$. In both cases essentially the full $3\times$ factor is realised.

\subsection*{Deployment Context}

EIP-4844 introduced KZG batch openings into Ethereum mainnet in March 2024. Each 12-second slot carries up to six data blobs, each committed via KZG and batch-verified using a zero polynomial over the BLS12-381 scalar field. Over 900{,}000 active validators perform this verification every slot. ZK-rollups built on Ethereum (zkSync Era, Polygon zkEVM, Scroll, Linea) additionally compute batch KZG openings in their provers during proof aggregation. The zero polynomial $Z(x)$ is computed repeatedly at chain lengths where the Delayed Inverse speedup is fully realised, making batch KZG opening one of the most impactful concrete applications of this paper's contribution.

\section{Conclusion}

We have presented the Delayed Inverse Algorithm, a simple but effective optimization for computing polynomial product chains over NTT-friendly rings. By deferring the inverse transform and performing all intermediate multiplications as Hadamard products in the spectral domain, the algorithm reduces the number of transforms from $3(k-1)$ to $k+1$, yielding a speedup ratio of $\frac{3(k-1)}{k+1}$ that approaches $3\times$ as the chain length grows. We further presented the Hybrid Delayed Inverse Product Tree, which achieves the same transform savings while reducing the critical path from $k+1$ to $\log_2 k + 2$ under parallel execution.

Our experimental results over the Goldilocks prime confirm the theoretical transform counts exactly and demonstrate measured speedups consistent with the predicted ratio. The optimization is applicable wherever polynomial product chains are evaluated over a fixed quotient ring with an NTT-friendly modulus --- a condition satisfied in batch KZG opening~\cite{kate2010kzg,eip4844}, where the zero polynomial $Z(x) = \prod_i(x-z_i)$ is computed over the BLS12-381 scalar field at chain lengths $t \geq 4096$.
...
\bibitem{eip4844}
V.~Buterin, D.~Feist, D.~Luo, G.~Santambrogio, M.~de~Miranda, and C.~Wahrstätter,
...
in \emph{Advances in Cryptology -- ASIACRYPT 2010}, ser. LNCS, vol. 6477,
